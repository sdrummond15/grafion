...
class MembersModelMember extends JModelAdmin
{
    ...
    {
        $form = $this->loadForm('com_members.member','member', array('control' => 'jform','load_data' => $loadData));
        if (empty($form))
        {
            return false;
        }
        ...
        if ($id != 0 && (!$user->authorise('core.edit.state', 'com_members.member.'.(int) $id))
            || ($id == 0 && !$user->authorise('core.edit.state', 'com_members'))
        )
        {
    ...
    protected function loadFormData()
    {
        ...
        $data = JFactory::getApplication()->getUserState('com_members.edit.member.data', array());
